from Nethya.Handlers.telethon import IMMUNE_USERS, telethn
from Nethya import REQUESTER
...
async def Nethya_is_admin(chat_id: int):
    status = False
    Nethya = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Nethya.id == user.id:
            status = True
            break
    return status
